...
class FunctionalShowHintEffect(FunctionalEffect):
    ...
    def triggerEffect(self):
        hint = self.getTarget()
        if hint is not None:
            if self._gui.playEffect('ShowHint', hint.getData()):
...
class FunctionalShowGreeting(FunctionalEffect):
    ...
    def isStillRunning(self):
        ...
        if arena is not None:
            result = arena.period in [ARENA_PERIOD.WAITING, ARENA_PERIOD.PREBATTLE]
            result or self._gui.stopEffect('ShowGreeting', self._effect.getTargetID())
        return result
