...
from gui.Scaleform.framework import AppRef, g_entitiesFactories, VIEW_TYPE
...
class SfLobbyProxy(GUIProxy, AppRef):
    ...
    def getSceneID(self):
        sceneID = None
        container = self.app.containerManager.getContainer(VIEW_TYPE.LOBBY_SUB)
    ...
    def isGuiDialogDisplayed(self):
        ...
        result = False
        if container is not None:
            dialogCount = container.getViewCount()
            if dialogCount > 0:
    ...
    def __onViewLoadInit(self, pyEntity):
        if pyEntity.settings.type is VIEW_TYPE.LOBBY_SUB:
