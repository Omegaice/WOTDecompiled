...
class ShopItem(FittingItem):
    ...
    def buy(self, count = 1, isShell = False, isCrew = True, crew_type = 0, isSlot = False, buyForCredits = False, callback = None):
        ...
        if buyForCredits and buyPrice[1] > 0 and (self.itemTypeName == 'shell' and shopRqs.isEnabledBuyingGoldShellsForCredits or self.itemTypeName == 'equipment'):
            buyGoldAmmoForCredits = shopRqs.isEnabledBuyingGoldEqsForCredits
            ...
            if (statsRqs.credits, statsRqs.gold) >= tuple(buyPrice):
                self.itemTypeName == 'vehicle' and BigWorld.player().shop.buyVehicle(self.descriptor.type.id[0], self.descriptor.type.id[1], isShell, isCrew, crew_type, callback=getBuyResponse)
            else:
...
class InventoryTankman(InventoryItem):
    ...
    def newSkillCount(self):
        if self.roleLevel == 100:
            hasNewSkill = self.lastSkillLevel == 100 or not len(self.skills)
            tmanDescr = hasNewSkill and TankmanDescr(self.compactDescr)
            i = 0
