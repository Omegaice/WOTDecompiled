...
def updateDossier4(version, dossierType, isExpaned, compDescr, unpackedData):
    ...
    if not isExpaned:
        ...
        data = unpackDossierCompDescr(layout, record_packing, compDescr)
    newVehDossiersCut = {}
    for vehTypeCompDescr, (battlesCount, wins) in data['vehDossiersCut'].iteritems():
        newVehDossiersCut[vehTypeCompDescr] = (battlesCount, wins, 0)

    data['vehDossiersCut'] = newVehDossiersCut
    data['_version'] = version
    return (data, compDescr)
...
def updateDossier14(version, dossierType, isExpaned, compDescr, unpackedData):
    ...
    if not isExpaned:
        ...
        data = unpackDossierCompDescr(layout, record_packing, compDescr)
    newVehDossiersCut = {}
    for vehTypeCompDescr, (battlesCount, wins, markOfMastery) in data['vehDossiersCut'].iteritems():
        newVehDossiersCut[vehTypeCompDescr] = (battlesCount,
         wins,
         markOfMastery,
         0)
...
def updateDossier15(version, dossierType, isExpaned, compDescr, unpackedData):
    ...
    if not isExpaned:
        ...
        data = unpackDossierCompDescr(layout, record_packing, compDescr)
    for record in ('treesCut', 'originalXP', 'damageAssistedTrack', 'damageAssistedRadio', 'mileage', 'shotsReceived', 'noDamageShotsReceived', 'piercedReceived', 'heHitsReceived', 'he_hits', 'pierced'):
        data[record] = 0
